...
public:
    ...
    bool loadObj(const std::string path, hittable_list& world, const shared_ptr<lambertian> mat, glm::mat4 transform) {
        ...
        while (std::getline(file, line)) {
            ...
            // === Parse faces ===
                ...
                if (indices.size() == 3) {
                    addTriangle(temp_vertices, indices[0], indices[1], indices[2], mat, world, transform);
                }

                // If face is a quad, split into two triangles
                else if (indices.size() == 4) {
                    addTriangle(temp_vertices, indices[0], indices[1], indices[2], mat, world, transform);
                    addTriangle(temp_vertices, indices[0], indices[2], indices[3], mat, world, transform);
                }
    ...
    /// Adds a triangle as a 4x4 matrix (homogeneous coordinates)
    void addTriangle(
        ...
        // === Add it to the world ===
        world.add(make_shared<triangle>(pos0, pos1, pos2, mat));
    }
